...
struct ObfuscatePass : public ModulePass {
  ...
  void obfuscateString(Module &M, Instruction *Inst, Value *Usr,
    ...
    auto Callee = M.getOrInsertFunction("__decrypt", FuncType);
    auto DecryptFunc = dyn_cast<Constant>(Callee.getCallee());
    ...
    auto Callee = M.getOrInsertFunction("__encrypt", FuncType);
    auto EncryptFunc = dyn_cast<Constant>(Callee.getCallee());
